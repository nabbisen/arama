// Shared test fixtures for the arama-cache integration test suite.

use std::io::Write;
use std::path::{Path, PathBuf};

use arama_cache::{
    CacheConfig, DbLocation, ImageCacheConfig, ImageCacheWriter, UpsertImageRequest,
    VideoCacheConfig, VideoCacheWriter,
};

// ---------------------------------------------------------------------------
// TempFile — self-cleaning temporary file
// ---------------------------------------------------------------------------

pub(super) struct TempFile {
    pub(super) path: PathBuf,
}

impl TempFile {
    pub(super) fn new(content: &[u8]) -> Self {
        let mut f = tempfile::NamedTempFile::new().unwrap();
        f.write_all(content).unwrap();
        let (_, path) = f.keep().unwrap();
        TempFile { path }
    }

    /// Create a temp file whose name ends with `suffix` (e.g. `".jpg"`).
    /// Required when the crate under test infers file format from the
    /// extension (as `image::open` does for JPEG content).
    pub(super) fn with_suffix(content: &[u8], suffix: &str) -> Self {
        let mut f = tempfile::Builder::new().suffix(suffix).tempfile().unwrap();
        f.write_all(content).unwrap();
        let (_, path) = f.keep().unwrap();
        TempFile { path }
    }

    pub(super) fn path(&self) -> &Path {
        &self.path
    }

    pub(super) fn overwrite(&self, content: &[u8]) {
        std::fs::write(&self.path, content).unwrap();
    }
}

impl Drop for TempFile {
    fn drop(&mut self) {
        let _ = std::fs::remove_file(&self.path);
    }
}

// ---------------------------------------------------------------------------
// Writer / DB factories
// ---------------------------------------------------------------------------

pub(super) fn tmp_db() -> tempfile::NamedTempFile {
    tempfile::NamedTempFile::new().unwrap()
}

pub(super) fn image_writer_with_db(db: &tempfile::NamedTempFile) -> ImageCacheWriter {
    ImageCacheWriter::as_session(ImageCacheConfig {
        cache_config: CacheConfig {
            db_location: DbLocation::Custom(db.path().to_path_buf()),
            read_conns: 2,
            thumbnail_dir: None,
        },
    })
    .unwrap()
}

pub(super) fn upsert_image(writer: &ImageCacheWriter, path: &Path) {
    writer
        .upsert(UpsertImageRequest {
            path: path.to_path_buf(),
            clip_vector: None,
        })
        .unwrap();
}

pub(super) fn video_writer() -> (VideoCacheWriter, tempfile::NamedTempFile) {
    let db = tmp_db();
    let w = VideoCacheWriter::as_session(VideoCacheConfig {
        cache_config: CacheConfig {
            db_location: DbLocation::Custom(db.path().to_path_buf()),
            read_conns: 2,
            thumbnail_dir: None,
        },
        ffmpeg_path: None,
    })
    .unwrap();
    (w, db)
}

/// Create `count` files with the given byte contents inside `dir`.
pub(super) fn files_in_dir(dir: &Path, contents: &[&[u8]]) -> Vec<PathBuf> {
    contents
        .iter()
        .enumerate()
        .map(|(i, c)| {
            let p = dir.join(format!("file_{i}.bin"));
            std::fs::write(&p, c).unwrap();
            p
        })
        .collect()
}

// ---------------------------------------------------------------------------
// Minimal valid JPEG (1×1 px) for thumbnail tests
// ---------------------------------------------------------------------------

pub(super) const MINIMAL_JPEG: &[u8] = &[
    ,
];
