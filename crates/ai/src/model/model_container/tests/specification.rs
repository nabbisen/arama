...
fn ready_for_single_file_model_requires_authenticated_manifest() {
    let (_scratch, model) = model_container(
        unique_model_name("single"),
...
fn ready_for_config_model_requires_safetensors_and_config() {
    let (_scratch, model) = model_container(
        unique_model_name("config"),
        SourceUrl::ModelSafetensorsConfigJson((
...
fn registry_rejects_conflicting_immutable_specifications_for_one_name() {
    ...
    let (_scratch_first, first) = single_file_model(
        name.clone(),
        "https://example.invalid/first".to_owned(),
        b"first",
    );
    let (_scratch_second, second) = single_file_model(
        name.clone(),
